...
batches = dataGen.flow(X_train, y_train,
                       batch_size=20)  # REQUESTING DATA GENERATOR TO GENERATE IMAGES  BATCH SIZE = NO. OF IMAGES
...
def myModel():
    ...
    model.add((Conv2D(no_Of_Filters, size_of_Filter, input_shape=(imageDimesions[0], imageDimesions[1], 1),
                      activation='relu')))  # ADDING MORE CONVOLUTION LAYERS= INCREASE ACCURACY
    ...
    model.compile(optamiser="adam", lr=0.001, loss='categorical_crossentropy', metrics=['accuracy'])
    return model
